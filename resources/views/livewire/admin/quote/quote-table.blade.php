            ...
            <div class="flex items-center">                
                <div class="items-center justify-between sm:flex">
                    <div class="flex items-center my-2 sm:my-0">
                        <span class="items-center justify-between sm:flex"> 
                            Show 
            ...
            <thead>
                <tr>
                    ...
                    <th class="text-gray-500 text-xs">
                        {{ __('cruds.quote.fields.message')}}
                        <span wire:click="sortBy('message')" class="float-right text-sm" style="cursor: pointer;">
                            <i class="fa fa-arrow-up {{ $sortColumnName === 'message' && $sortDirection === 'asc' ? '' : 'text-muted' }}"></i>
                            <i class="fa fa-arrow-down m-0 {{ $sortColumnName === 'message' && $sortDirection === 'desc' ? '' : 'text-muted' }}"></i>
                        </span>
                    </th>
            ...
            <tbody>
                ...
                @foreach($allQuote as $serialNo => $quote)
                    @php 
                        ...
                        $deadline = Carbon\Carbon::parse($quote->created_at)->startOfDay();
                    @endphp
                    <tr>
                        <td>{{ $serialNo+1 }}</td>
                        <td>{{ ucwords($quote->message) }}</td>
                        <td>{{ convertDateTimeFormat($quote->created_at,'date') }}</td>
                        <td>

                            @can('quote_show')
                            <button title="Show" type="button" wire:click.prevent="$emitUp('show', {{$quote->id}})" class="btn btn-primary btn-rounded btn-icon">
                                <i class="ti-eye"></i>
                            </button>
                            @endcan

                            @can('quote_edit')
                                @if($deadline->eq($currentDate))
                                    <button title="Edit" type="button" wire:click.prevent="$emitUp('edit', {{$quote->id}})" class="btn btn-info btn-rounded btn-icon">
                            ...
                            @can('quote_delete')
                            <button title="Delete" type="button" wire:click.prevent="$emitUp('delete', {{$quote->id}})" class="btn btn-danger btn-rounded btn-icon">
                                <i class="ti-trash"></i>
                            </button>
                            @endcan
                ...
                <li>
                    <div class="webinar-item">
                        <p class="quotes-content">
                            Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellat suscipit fuga accusantium fugiat ex beatae vel reiciendis nemo quae laudantium odit, deserunt id, quaerat praesentium corporis veniam quibusdam veritatis natus. Lorem, ipsum dolor sit amet consectetur adipisicing elit. Repellat suscipit fuga accusantium fugiat ex beatae vel reiciendis nemo quae laudantium odit, deserunt id, quaerat praesentium corporis veniam quibusdam veritatis natus.
                        </p>
                        ...
                        <div class="update-webinar">
                            <ul class="d-flex">
                                                                <li>
                                    <a href="javascript:void()" wire:click.prevent="$emitUp('edit', 3)">
                                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <path d="M16.5413 8.19173C16.483 8.19173 16.4246 8.1834 16.3746 8.16673C14.183 7.55006 12.4413 5.8084 11.8246 3.61673C11.733 3.2834 11.9246 2.94173 12.258 2.84173C12.5913 2.75006 12.933 2.94173 13.0246 3.27506C13.5246 5.05006 14.933 6.4584 16.708 6.9584C17.0413 7.05006 17.233 7.40006 17.1413 7.7334C17.0663 8.01673 16.8163 8.19173 16.5413 8.19173Z" fill="#40658B"></path>
                                                                ...
                                                                <li>
                                    <a href="javascript:void()" wire:click.prevent="$emitUp('delete', 3)">
                                    <svg xmlns="http://www.w3.org/2000/svg" id="Outline" fill="none" viewBox="0 0 24 24" width="20" height="20"><path d="M21,4H17.9A5.009,5.009,0,0,0,13,0H11A5.009,5.009,0,0,0,6.1,4H3A1,1,0,0,0,3,6H4V19a5.006,5.006,0,0,0,5,5h6a5.006,5.006,0,0,0,5-5V6h1a1,1,0,0,0,0-2ZM11,2h2a3.006,3.006,0,0,1,2.829,2H8.171A3.006,3.006,0,0,1,11,2Zm7,17a3,3,0,0,1-3,3H9a3,3,0,0,1-3-3V6H18Z" fill="#40658B"></path><path d="M10,18a1,1,0,0,0,1-1V11a1,1,0,0,0-2,0v6A1,1,0,0,0,10,18Z" fill="#40658B"></path><path d="M14,18a1,1,0,0,0,1-1V11a1,1,0,0,0-2,0v6A1,1,0,0,0,14,18Z" fill="#40658B"></path></svg>
                                    </a>
                                </li>
                                
                                                                <li>
                                    <a href="javascript:void()" wire:click.prevent="$emitUp('show', 3)">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" width="20" height="20"><g id="_01_align_center" data-name="01 align center"><path d="M23.821,11.181v0C22.943,9.261,19.5,3,12,3S1.057,9.261.179,11.181a1.969,1.969,0,0,0,0,1.64C1.057,14.739,4.5,21,12,21s10.943-6.261,11.821-8.181A1.968,1.968,0,0,0,23.821,11.181ZM12,19c-6.307,0-9.25-5.366-10-6.989C2.75,10.366,5.693,5,12,5c6.292,0,9.236,5.343,10,7C21.236,13.657,18.292,19,12,19Z" fill="#40658B"></path><path d="M12,7a5,5,0,1,0,5,5A5.006,5.006,0,0,0,12,7Zm0,8a3,3,0,1,1,3-3A3,3,0,0,1,12,15Z" fill="#40658B"></path></g></svg>
                                    </a>
                                </li>
                                                            </ul>
                        </div>
